...
import { getMedicalRecordList, addMedicalRecordPlan, getMedicalRecordInfo, updateMedicalRecordInfo } from "@/api/patientDetails";
...
const MedicalRecord = ({ id }) => {
  ...
  const fetchData = async () => {
    setListLoading(true);
    const result = await getMedicalRecordList({ ...listQuery, patient_id: parseInt(id) });
  ...
  const handleAdd = async () => {
    ...
    const addResult = await addMedicalRecordPlan({ ...baseData, patient_id: parseInt(id), content: '', edit_time: moment().unix() });
    if (addResult) {
  ...
  const onFinish = async (values) => {
    ...
    const result = await updateMedicalRecordInfo({ ...values, id: currentId });
    if (result) {
  ...
  const handleClickItem = async (item, index) => {
    ...
    setInfoLoading(true);
    const result = await getMedicalRecordInfo({ id: item.id });
