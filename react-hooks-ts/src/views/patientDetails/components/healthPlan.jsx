...
import { getHealthPlanList, addHealthPlanPlan, getHealthPlanInfo, updateHealthPlanInfo } from "@/api/patientDetails";
...
const HealthPlan = ({ id }) => {
  ...
  const fetchData = async () => {
    setListLoading(true);
    const result = await getHealthPlanList({ ...listQuery, patient_id: parseInt(id) });
  ...
  const handleAdd = async () => {
    ...
    const addResult = await addHealthPlanPlan({ ...baseData, patient_id: parseInt(id), content: '', edit_time: moment().unix() });
    if (addResult) {
  ...
  const onFinish = async (values) => {
    ...
    const result = await updateHealthPlanInfo({ ...values, id: currentId });
    if (result) {
  ...
  const handleClickItem = async (item, index) => {
    ...
    setInfoLoading(true);
    const result = await getHealthPlanInfo({ id: item.id });
          ...
          >
            ...
            <Spin spinning={infoLoading} wrapperClassName="print-content">
              ...
              <div id="print" className="print-content">
                <Title level={3} style={{ textAlign: 'center' }} id="catelog1">保健计划</Title>
