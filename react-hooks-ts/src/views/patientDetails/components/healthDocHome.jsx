...
const HealthDocHome = ({ id }) => {
  ...
  const fetchData = async () => {
    ...
    const renderInfo = await getHealthDocInfo({ patient_id: parseInt(id) });
  ...
  const onFinish = async (values) => {
    ...
    const renderResult = await updateHealthDocInfo({ patient_id: parseInt(id), big_events, diagnosis, now_drug, union_diagnosis, check, hospitalization, vaccines_use });
          ...
          >
            ...
            <Spin spinning={loading} wrapperClassName="print-content">
              ...
              <div id="print" className="">
                <Title level={3} style={{ textAlign: 'center' }} id="catelog1">健康档案首页</Title>
                  ...
                  <div style={{ display: 'flex', marginTop: '10px' }}>
                    ...
                    <div style={{ width: '50%' }}>
                      ...
                      <Form.Item name="birth_address" style={{ display: 'inline-block', verticalAlign: 'baseline', margin: '0' }}>
                        <Input disabled={isReview} style={{ width: '300px', borderTop: 'none', borderLeft: 'none', borderRight: 'none' }} />
                      </Form.Item>
                    </div>
                  </div>
                  <hr />
