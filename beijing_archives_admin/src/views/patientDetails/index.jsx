...
const Details = ({ match, location, menu }) => {
      ...
      <div className="menu-list">
        ...
        >
          ...
          <SubMenu key="2" title="病案管理">
            {menu.includes('1-1') ? <Menu.Item key="20">病案首页</Menu.Item> : null}
            {menu.includes('1-2') ? <Menu.Item key="21">大事记</Menu.Item> : null}
            {menu.includes('1-3') ? <Menu.Item key="22">急救预案</Menu.Item> : null}
            {menu.includes('1-4') ? <Menu.Item key="23">保健计划</Menu.Item> : null}
            {menu.includes('1-5') ? <Menu.Item key="24">病历记录</Menu.Item> : null}
            {menu.includes('1-6') ? <Menu.Item key="25">病情总结</Menu.Item> : null}
            {menu.includes('1-7') ? <Menu.Item key="26">访视观察</Menu.Item> : null}
            {menu.includes('1-8') ? <Menu.Item key="27">出诊记录单</Menu.Item> : null}
          </SubMenu>
          {
            menu.includes('1-9') ?
              <Menu.Item key="3">
...
export default connect((state) => state.user, null)(Details);
