...
service.interceptors.request.use(
  (config: any) => {
    ...
    if (config.data) {
      // config.data = { data: secret.encryption(config.data, str, str) };
    }
    return config;
  },
  (error: any) => {
...
service.interceptors.response.use(
  ...
  (response: any) => {
    ...
    if (res.code === 500 || res.code === 601 || res.code === 602 || res.code === 603 || res.code === 604) {
      ...
      // res.data = secret.decryption(res.data, str, str);
      return res;
    }
  },
  (error: any) => {
