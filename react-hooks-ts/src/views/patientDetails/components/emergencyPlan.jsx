...
import { getEmergencyPlanList, addEmergencyPlan, getEmergencyInfo, updateEmergencyInfo } from "@/api/patientDetails";
...
const EmergencyPlan = ({ id }) => {
  ...
  const fetchData = async () => {
    setListLoading(true);
    const result = await getEmergencyPlanList({ ...listQuery, patient_id: parseInt(id) });
  ...
  const handleAdd = async () => {
    ...
    const addResult = await addEmergencyPlan({ ...baseData, patient_id: parseInt(id), content: '', edit_time: moment().unix() });
    if (addResult) {
  ...
  const onFinish = async (values) => {
    ...
    const result = await updateEmergencyInfo({ ...values, id: currentId });
    if (result) {
  ...
  const handleClickItem = async (item, index) => {
    ...
    setInfoLoading(true);
    const result = await getEmergencyInfo({ id: item.id });
