...
const FinishCashout = ({ token, name, id, visible, onCancel, onOk, confirmLoading, finishCashoutInfo }) => {
  ...
  const [cashoutFileList, setCashoutFileList] = useState([]);
  ...
  const props = {
    ...
    onChange(info) {
      ...
      if (info.file.status === 'done') {
        ...
        let cashoutFileListTemp = [...cashoutFileList];
        cashoutFileListTemp.push(fileObj);
        setCashoutFileList(cashoutFileListTemp);
  ...
  return (
    <Modal
      ...
      onOk={() => {
        form.validateFields()
          .then((values) => {
            values.payment_attachment = JSON.stringify(cashoutFileList);
            onOk(values);
          })
          .catch((info) => {
    ...
    >
      <Descriptions title="收款账户信息" column={3}>
        <Descriptions.Item label="开户行">{finishCashoutInfo.accept_bank ? finishCashoutInfo.accept_bank : '-'}</Descriptions.Item>
        <Descriptions.Item label="户主">{finishCashoutInfo.accept_contact ? finishCashoutInfo.accept_contact : '-'}</Descriptions.Item>
        <Descriptions.Item label="账号">{finishCashoutInfo.accept_account ? finishCashoutInfo.accept_account : '-'}</Descriptions.Item>
        <Descriptions.Item label="总打款金额"><span style={{ fontSize: '20px' }}>{finishCashoutInfo.crash_money ? formartMoney(finishCashoutInfo.crash_money) : '-'}</span></Descriptions.Item>
      </Descriptions>
...
export default connect((state) => state.user, null)(FinishCashout);
